// https://api.themoviedb.org/3/discover/movie?api_key=1b7c076a0e4849aeefd1f3c429c99a36&with_genres=99
// documentarys
const bodys =  document.querySelector("body");
bodys.style.backgroundColor = "#1F1F1F";

const documentaryUrl = 'https://api.themoviedb.org/3/discover/movie?api_key=1b7c076a0e4849aeefd1f3c429c99a36&with_genres=99';
const documentaryBody = document.querySelector('.movie-documentary-body');
...
const title = document.querySelector(".documentary-title");

const movieDocumentaryAPI = async (url)=>{
      const sendAPI = await fetch(url);
      const getAPIData = await sendAPI.json();
      const useAPIResults = getAPIData.results;
      documentaryBody.innerHTML = '';
      useAPIResults.map((list)=>{
        extractDocumentaryAPI(list);
      })

}
movieDocumentaryAPI(documentaryUrl);

const extractDocumentaryAPI = (movies)=>{
      const {original_title,release_date,overview,poster_path} = movies;
      const documentaryList = document.createElement('div');
      const setHorrorClass = documentaryList.setAttribute("class","horror");
      documentaryList.innerHTML = `
      <div class="movie-documentary">
          <div class="movie-documentary-poster">
               <img width="200px" src = ${apiImages + poster_path} alt= ${original_title}>
          </div>
          <div class="movie-documentary-detail">
          <h3>${original_title}</h3>
          <span>${release_date}</span>
      </div>  
    `
    documentaryBody.appendChild(documentaryList);
}
...
form.addEventListener("submit",(s)=>{
    ...
    if(inputValue !== ''){
        movieDocumentaryAPI(searchAPIQuery + inputValue);
...
mobileForm.addEventListener("submit",(t)=>{
       ...
       if(mobileValue !== ''){
        movieDocumentaryAPI(searchAPIQuery + mobileValue);
