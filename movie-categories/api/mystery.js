// https://api.themoviedb.org/3/discover/movie?api_key=1b7c076a0e4849aeefd1f3c429c99a36&with_genres=9648
// mysterys
const bodys =  document.querySelector("body");
bodys.style.backgroundColor = "#1F1F1F";

const mysteryUrl = 'https://api.themoviedb.org/3/discover/movie?api_key=1b7c076a0e4849aeefd1f3c429c99a36&with_genres=9648';
const mysteryBody = document.querySelector('.movie-mystery-body');
...
const title = document.querySelector(".mystery-title");

const movieMysteryAPI = async (url)=>{
      ...
      useAPIResults.map((list)=>{
        extractMysteryAPI(list);
      })

}
movieMysteryAPI(mysteryUrl);

const extractMysteryAPI = (movies)=>{
      const {original_title,release_date,overview,poster_path} = movies;
      const mysteryList = document.createElement('div');
      const setHorrorClass = mysteryList.setAttribute("class","horror");
      mysteryList.innerHTML = `
      <div class="movie-mystery">
          <div class="movie-mystery-poster">
               <img width="200px" src = ${apiImages + poster_path} alt= ${original_title}>
          </div>
          <div class="movie-mystery-detail">
          <h3>${original_title}</h3>
          <span>${release_date}</span>
      </div>  
    `
    mysteryBody.appendChild(mysteryList);
}
...
form.addEventListener("submit",(s)=>{
    ...
    if(inputValue !== ''){
        movieMysteryAPI(searchAPIQuery + inputValue);
...
mobileForm.addEventListener("submit",(t)=>{
       ...
       if(mobileValue !== ''){
        movieMysteryAPI(searchAPIQuery + mobileValue);
