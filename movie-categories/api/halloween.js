// https://api.themoviedb.org/3/discover/movie?api_key=1b7c076a0e4849aeefd1f3c429c99a36&with_genres=53
//  halloween
const bodys =  document.querySelector("body");
bodys.style.backgroundColor = "#1F1F1F";

const halloweenUrl = 'https://api.themoviedb.org/3/discover/movie?api_key=1b7c076a0e4849aeefd1f3c429c99a36&with_genres=53';
const halloweenBody = document.querySelector('.movie-halloween-body');
...
const title = document.querySelector(".halloween-title");


const movieHalloweenAPI = async (url)=>{
      const sendAPI = await fetch(url);
      const getAPIData = await sendAPI.json();
      const useAPIResults = getAPIData.results;
      halloweenBody.innerHTML  = '';
      useAPIResults.map((list)=>{
        extractHalloweenAPI(list);
      })

}
movieHalloweenAPI(halloweenUrl);

const extractHalloweenAPI = (movies)=>{
      const {original_title,release_date,overview,poster_path} = movies;
      const halloweenList = document.createElement('div');
      const setHorrorClass = halloweenList.setAttribute("class","horror");
      halloweenList.innerHTML = `
      <div class="movie-halloween">
          <div class="movie-halloween-poster">
               <img width="200px" src = ${apiImages + poster_path} alt= ${original_title}>
          </div>
          <div class="movie-halloween-detail">
          <h3>${original_title}</h3>
          <span>${release_date}</span>
      </div>  
    `
    halloweenBody.appendChild(halloweenList);
}
...
form.addEventListener("submit",(s)=>{
    ...
    if(inputValue !== ''){
        movieHalloweenAPI(searchAPIQuery + inputValue);
...
mobileForm.addEventListener("submit",(t)=>{
       ...
       if(mobileValue !== ''){
        movieHalloweenAPI(searchAPIQuery + mobileValue);
