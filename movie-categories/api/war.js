// https://api.themoviedb.org/3/discover/movie?api_key=1b7c076a0e4849aeefd1f3c429c99a36&with_genres=10752
// wars
const bodys =  document.querySelector("body");
bodys.style.backgroundColor = "#1F1F1F";

const warUrl = 'https://api.themoviedb.org/3/discover/movie?api_key=1b7c076a0e4849aeefd1f3c429c99a36&with_genres=10752';
const warBody = document.querySelector('.movie-war-body');
...
const title = document.querySelector(".war-title");

const moviewarAPI = async (url)=>{
...
const extractwarAPI = (movies)=>{
      const {original_title,release_date,overview,poster_path} = movies;
      const warList = document.createElement('div');
      const setHorrorClass = warList.setAttribute("class","horror");
      warList.innerHTML = `
      <div class="movie-war">
          <div class="movie-war-poster">
               <img width="200px" src = ${apiImages + poster_path} alt= ${original_title}>
          </div>
          <div class="movie-war-detail">
          <h3>${original_title}</h3>
          <span>${release_date}</span>
      </div>  
    `
    warBody.appendChild(warList);
}
form.addEventListener("submit",(s)=>{
    s.preventDefault();
    const inputValue  = input.value;
    if(inputValue !== ''){
        moviewarAPI(searchAPIQuery + inputValue);
...
mobileForm.addEventListener("submit",(t)=>{
       ...
       if(mobileValue !== ''){
        moviewarAPI(searchAPIQuery + mobileValue);
