// https://api.themoviedb.org/3/discover/movie?api_key=1b7c076a0e4849aeefd1f3c429c99a36&with_genres=18
// dramas
const bodys =  document.querySelector("body");
bodys.style.backgroundColor = "#1F1F1F";

const dramaUrl = 'https://api.themoviedb.org/3/discover/movie?api_key=1b7c076a0e4849aeefd1f3c429c99a36&with_genres=18';
const dramaBody = document.querySelector('.movie-drama-body');
...
const title = document.querySelector(".drama-title");

const moviedramaAPI = async (url)=>{
      ...
      useAPIResults.map((list)=>{
        extractdramaAPI(list);
      })

}
moviedramaAPI(dramaUrl);

const extractdramaAPI = (movies)=>{
      const {original_title,release_date,overview,poster_path} = movies;
      const dramaList = document.createElement('div');
      const setHorrorClass = dramaList.setAttribute("class","horror");
      dramaList.innerHTML = `
      <div class="movie-drama">
          <div class="movie-drama-poster">
               <img width="200px" src = ${apiImages + poster_path} alt= ${original_title}>
          </div>
          <div class="movie-drama-detail">
          <h3>${original_title}</h3>
          <span>${release_date}</span>
      </div>  
    `
    dramaBody.appendChild(dramaList);
}
...
form.addEventListener("submit",(s)=>{
    ...
    if(inputValue !== ''){
        moviedramaAPI(searchAPIQuery + inputValue);
...
mobileForm.addEventListener("submit",(t)=>{
       ...
       if(mobileValue !== ''){
        moviedramaAPI(searchAPIQuery + mobileValue);
