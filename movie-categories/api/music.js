// https://api.themoviedb.org/3/discover/movie?api_key=1b7c076a0e4849aeefd1f3c429c99a36&with_genres=10402
// musics
const bodys =  document.querySelector("body");
bodys.style.backgroundColor = "#1F1F1F";

const musicUrl = 'https://api.themoviedb.org/3/discover/movie?api_key=1b7c076a0e4849aeefd1f3c429c99a36&with_genres=10402';
const musicBody = document.querySelector('.movie-music-body');
...
const movieMusicAPI = async (url)=>{
      ...
      useAPIResults.map((list)=>{
        extractMusicAPI(list);
      })

}
movieMusicAPI(musicUrl);

const extractMusicAPI = (movies)=>{
      const {original_title,release_date,overview,poster_path} = movies;
      const musicList = document.createElement('div');
      const setHorrorClass = musicList.setAttribute("class","horror");
      musicList.innerHTML = `
      <div class="movie-music">
          <div class="movie-music-poster">
               <img width="200px" src = ${apiImages + poster_path} alt= ${original_title}>
          </div>
          <div class="movie-music-detail">
          <h3>${original_title}</h3>
          <span>${release_date}</span>
      </div>  
    `
    musicBody.appendChild(musicList);
}
...
form.addEventListener("submit",(s)=>{
    ...
    if(inputValue !== ''){
        movieMusicAPI(searchAPIQuery + inputValue);
...
mobileForm.addEventListener("submit",(t)=>{
       ...
       if(mobileValue !== ''){
        movieMusicAPI(searchAPIQuery + mobileValue);
