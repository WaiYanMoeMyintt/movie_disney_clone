// https://api.themoviedb.org/3/discover/movie?api_key=1b7c076a0e4849aeefd1f3c429c99a36&with_genres=35
// comedys
const bodys =  document.querySelector("body");
bodys.style.backgroundColor = "#1F1F1F";

const comedyUrl = 'https://api.themoviedb.org/3/discover/movie?api_key=1b7c076a0e4849aeefd1f3c429c99a36&with_genres=35';
const comedyBody = document.querySelector('.movie-comedy-body');
...
const title = document.querySelector(".comedy-title");

const movieComedyAPI = async (url)=>{
      const sendAPI = await fetch(url);
      const getAPIData = await sendAPI.json();
      const useAPIResults = getAPIData.results;
      comedyBody.innerHTML = '';
      useAPIResults.map((list)=>{
        extractComedyAPI(list);
      })

}
movieComedyAPI(comedyUrl);

const extractComedyAPI = (movies)=>{
      const {original_title,release_date,overview,poster_path} = movies;
      const comedyList = document.createElement('div');
      const setHorrorClass = comedyList.setAttribute("class","horror");
      comedyList.innerHTML = `
      <div class="movie-comedy">
          <div class="movie-comedy-poster">
               <img width="200px" src = ${apiImages + poster_path} alt= ${original_title}>
          </div>
          <div class="movie-comedy-detail">
          <h3>${original_title}</h3>
          <span>${release_date}</span>
      </div>  
    `
    comedyBody.appendChild(comedyList);
}
...
form.addEventListener("submit",(s)=>{
    ...
    if(inputValue !== ''){
        movieComedyAPI(searchAPIQuery + inputValue);
...
mobileForm.addEventListener("submit",(t)=>{
       ...
       if(mobileValue !== ''){
        movieComedyAPI(searchAPIQuery + mobileValue);
