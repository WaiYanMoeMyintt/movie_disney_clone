// https://api.themoviedb.org/3/movie/now_playing?api_key=1b7c076a0e4849aeefd1f3c429c99a36&language=en-US&page=1
// movieNows
const bodys =  document.querySelector("body");
bodys.style.backgroundColor = "#1F1F1F";

const movieNowUrl = 'https://api.themoviedb.org/3/movie/now_playing?api_key=1b7c076a0e4849aeefd1f3c429c99a36&language=en-US&page=1';
const movieNowBody = document.querySelector('.movie-now-playing-body');
...
const movieNowAPI = async (url)=>{
      const sendAPI = await fetch(url);
      const getAPIData = await sendAPI.json();
      const useAPIResults = getAPIData.results;
      movieNowBody.innerHTML = '';
      useAPIResults.map((list)=>{
        extractNowAPI(list);
      })

}
movieNowAPI(movieNowUrl);

const extractNowAPI = (movies)=>{
      const {original_title,release_date,overview,poster_path} = movies;
      const movieNowList = document.createElement('div');
      const setHorrorClass = movieNowList.setAttribute("class","horror");
      movieNowList.innerHTML = `
      <div class="movie-now-playing">
          <div class="movie-now-playing-poster">
               <img width="200px" src = ${apiImages + poster_path} alt= ${original_title}>
          </div>
          <div class="movie-now-playing-detail">
          <h3>${original_title}</h3>
          <span>${release_date}</span>
      </div>  
    `
    movieNowBody.appendChild(movieNowList);
}
...
form.addEventListener("submit",(s)=>{
    ...
    if(inputValue !== ''){
        movieNowAPI(searchAPIQuery + inputValue);
...
mobileForm.addEventListener("submit",(t)=>{
       ...
       if(mobileValue !== ''){
        movieNowAPI(searchAPIQuery + mobileValue);
