// https://api.themoviedb.org/3/discover/movie?api_key=1b7c076a0e4849aeefd1f3c429c99a36&with_genres=16
// cartoons
const bodys =  document.querySelector("body");
bodys.style.backgroundColor = "#1F1F1F";

const cartoonUrl = 'https://api.themoviedb.org/3/discover/movie?api_key=1b7c076a0e4849aeefd1f3c429c99a36&with_genres=16';
const cartoonBody = document.querySelector('.movie-cartoon-body');
...
const title = document.querySelector(".cartoon-title");

const moviecartoonAPI = async (url)=>{
      const sendAPI = await fetch(url);
      const getAPIData = await sendAPI.json();
      const useAPIResults = getAPIData.results;
      cartoonBody.innerHTML = '';
      useAPIResults.map((list)=>{
        extractcartoonAPI(list);
      })

}
moviecartoonAPI(cartoonUrl);

const extractcartoonAPI = (movies)=>{
      const {original_title,release_date,overview,poster_path} = movies;
      const cartoonList = document.createElement('div');
      const setHorrorClass = cartoonList.setAttribute("class","horror");
      cartoonList.innerHTML = `
      <div class="movie-cartoon">
          <div class="movie-cartoon-poster">
               <img width="200px" src = ${apiImages + poster_path} alt= ${original_title}>
          </div>
          <div class="movie-cartoon-detail">
          <h3>${original_title}</h3>
          <span>${release_date}</span>
      </div>  
    `
    cartoonBody.appendChild(cartoonList);
}
...
form.addEventListener("submit",(s)=>{
    ...
    if(inputValue !== ''){
        moviecartoonAPI(searchAPIQuery + inputValue);
...
mobileForm.addEventListener("submit",(t)=>{
       ...
       if(mobileValue !== ''){
        moviecartoonAPI(searchAPIQuery + mobileValue);
