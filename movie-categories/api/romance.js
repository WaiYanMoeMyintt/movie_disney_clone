// https://api.themoviedb.org/3/discover/movie?api_key=1b7c076a0e4849aeefd1f3c429c99a36&with_genres=10749
// romances
const bodys =  document.querySelector("body");
bodys.style.backgroundColor = "#1F1F1F";

const romanceUrl = 'https://api.themoviedb.org/3/discover/movie?api_key=1b7c076a0e4849aeefd1f3c429c99a36&with_genres=10749';
const romanceBody = document.querySelector('.movie-romance-body');
...
const title = document.querySelector(".romance-title");

const movieRomanceAPI = async (url)=>{
      const sendAPI = await fetch(url);
      const getAPIData = await sendAPI.json();
      const useAPIResults = getAPIData.results;
      romanceBody.innerHTML = '';
      useAPIResults.map((list)=>{
        extractRomanceAPI(list);
      })

}
movieRomanceAPI(romanceUrl);

const extractRomanceAPI = (movies)=>{
      const {original_title,release_date,overview,poster_path} = movies;
      const romanceList = document.createElement('div');
      const setHorrorClass = romanceList.setAttribute("class","horror");
      romanceList.innerHTML = `
      <div class="movie-romance">
          <div class="movie-romance-poster">
               <img width="200px" src = ${apiImages + poster_path} alt= ${original_title}>
          </div>
          <div class="movie-romance-detail">
          <h3>${original_title}</h3>
          <span>${release_date}</span>
      </div>  
    `
    romanceBody.appendChild(romanceList);
}
...
form.addEventListener("submit",(s)=>{
    ...
    if(inputValue !== ''){
        movieRomanceAPI(searchAPIQuery + inputValue);
...
mobileForm.addEventListener("submit",(t)=>{
       ...
       if(mobileValue !== ''){
        movieRomanceAPI(searchAPIQuery + mobileValue);
