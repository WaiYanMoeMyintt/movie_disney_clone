// https://api.themoviedb.org/3/discover/movie?api_key=1b7c076a0e4849aeefd1f3c429c99a36&with_genres=878
// scifis
const bodys =  document.querySelector("body");
bodys.style.backgroundColor = "#1F1F1F";

const scifiUrl = 'https://api.themoviedb.org/3/discover/movie?api_key=1b7c076a0e4849aeefd1f3c429c99a36&with_genres=878';
const scifiBody = document.querySelector('.movie-scifi-body');
...
const title = document.querySelector(".scifi-title");

const movieSciFiAPI = async (url)=>{
      ...
      useAPIResults.map((list)=>{
        extractSciFiAPI(list);
      })

}
movieSciFiAPI(scifiUrl);

const extractSciFiAPI = (movies)=>{
      const {original_title,release_date,overview,poster_path} = movies;
      const scifiList = document.createElement('div');
      const setHorrorClass = scifiList.setAttribute("class","horror");
      scifiList.innerHTML = `
      <div class="movie-scifi">
          <div class="movie-scifi-poster">
               <img width="200px" src = ${apiImages + poster_path} alt= ${original_title}>
          </div>
          <div class="movie-scifi-detail">
          <h3>${original_title}</h3>
          <span>${release_date}</span>
      </div>  
    `
    scifiBody.appendChild(scifiList);
}
...
form.addEventListener("submit",(s)=>{
    ...
    if(inputValue !== ''){
        movieSciFiAPI(searchAPIQuery + inputValue);
...
mobileForm.addEventListener("submit",(t)=>{
       ...
       if(mobileValue !== ''){
        movieSciFiAPI(searchAPIQuery + mobileValue);
