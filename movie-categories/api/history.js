// https://api.themoviedb.org/3/discover/movie?api_key=1b7c076a0e4849aeefd1f3c429c99a36&with_genres=36
// historys
const bodys =  document.querySelector("body");
bodys.style.backgroundColor = "#1F1F1F";

const historyUrl = 'https://api.themoviedb.org/3/discover/movie?api_key=1b7c076a0e4849aeefd1f3c429c99a36&with_genres=36';
const historyBody = document.querySelector('.movie-history-body');
...
const movieHistoryAPI = async (url)=>{
      ...
      useAPIResults.map((list)=>{
        extractHistoryAPI(list);
      })

}
movieHistoryAPI(historyUrl);

const extractHistoryAPI = (movies)=>{
      const {original_title,release_date,overview,poster_path} = movies;
      const historyList = document.createElement('div');
      const setHorrorClass = historyList.setAttribute("class","horror");
      historyList.innerHTML = `
      <div class="movie-history">
          <div class="movie-history-poster">
               <img width="200px" src = ${apiImages + poster_path} alt= ${original_title}>
          </div>
          <div class="movie-history-detail">
...
form.addEventListener("submit",(s)=>{
    ...
    if(inputValue !== ''){
        movieHistoryAPI(searchAPIQuery + inputValue);
...
mobileForm.addEventListener("submit",(t)=>{
       ...
       if(mobileValue !== ''){
        movieHistoryAPI(searchAPIQuery + mobileValue);
