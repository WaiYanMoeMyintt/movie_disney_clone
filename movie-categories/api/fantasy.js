// https://api.themoviedb.org/3/discover/movie?api_key=1b7c076a0e4849aeefd1f3c429c99a36&with_genres=14
// fantasys
const bodys =  document.querySelector("body");
bodys.style.backgroundColor = "#1F1F1F";

const fantasyUrl = 'https://api.themoviedb.org/3/discover/movie?api_key=1b7c076a0e4849aeefd1f3c429c99a36&with_genres=14';
const fantasyBody = document.querySelector('.movie-fantasy-body');
...
const title = document.querySelector(".fantasy-title");

const moviefantasyAPI = async (url)=>{
      const sendAPI = await fetch(url);
      const getAPIData = await sendAPI.json();
      const useAPIResults = getAPIData.results;
      fantasyBody.innerHTML = '';
      useAPIResults.map((list)=>{
        extractfantasyAPI(list);
      })

}
moviefantasyAPI(fantasyUrl);

const extractfantasyAPI = (movies)=>{
      const {original_title,release_date,overview,poster_path} = movies;
      const fantasyList = document.createElement('div');
      const setHorrorClass = fantasyList.setAttribute("class","horror");
      fantasyList.innerHTML = `
      <div class="movie-fantasy">
          <div class="movie-fantasy-poster">
               <img width="200px" src = ${apiImages + poster_path} alt= ${original_title}>
          </div>
          <div class="movie-fantasy-detail">
          <h3>${original_title}</h3>
          <span>${release_date}</span>
      </div>  
    `
    fantasyBody.appendChild(fantasyList);
}
form.addEventListener("submit",(s)=>{
    s.preventDefault();
    const inputValue  = input.value;
    if(inputValue !== ''){
        moviefantasyAPI(searchAPIQuery + inputValue);
...
mobileForm.addEventListener("submit",(t)=>{
       ...
       if(mobileValue !== ''){
        moviefantasyAPI(searchAPIQuery + mobileValue);
