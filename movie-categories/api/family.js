// https://api.themoviedb.org/3/discover/movie?api_key=1b7c076a0e4849aeefd1f3c429c99a36&with_genres=10751
// familys
const bodys =  document.querySelector("body");
bodys.style.backgroundColor = "#1F1F1F";

const familyUrl = 'https://api.themoviedb.org/3/discover/movie?api_key=1b7c076a0e4849aeefd1f3c429c99a36&with_genres=10751';
const familyBody = document.querySelector('.movie-family-body');
...
const movieFamilyAPI = async (url)=>{
      ...
      useAPIResults.map((list)=>{
        extractFamilyAPI(list);
      })

}
movieFamilyAPI(familyUrl);

const extractFamilyAPI = (movies)=>{
      const {original_title,release_date,overview,poster_path} = movies;
      const familyList = document.createElement('div');
      const setHorrorClass = familyList.setAttribute("class","horror");
      familyList.innerHTML = `
      <div class="movie-family">
          <div class="movie-family-poster">
               <img width="200px" src = ${apiImages + poster_path} alt= ${original_title}>
          </div>
          <div class="movie-family-detail">
          <h3>${original_title}</h3>
          <span>${release_date}</span>
      </div>  
    `
    familyBody.appendChild(familyList);
}
...
form.addEventListener("submit",(s)=>{
    ...
    if(inputValue !== ''){
        movieFamilyAPI(searchAPIQuery + inputValue);
...
mobileForm.addEventListener("submit",(t)=>{
       ...
       if(mobileValue !== ''){
        movieFamilyAPI(searchAPIQuery + mobileValue);
